...
async function uploadFileToDataPower(filePath, domain, connectionDetails) {
	...
	try {
		...
		const deletedFileResponse = await axios.delete(`${connectionDetails.socket}/mgmt/filestore/${domain}/local/${fileName}`, {
			headers: {
	...
	try {
		console.log(`${connectionDetails.socket}/mgmt/filestore/${domain}/local/${fileName}`)
		const fileExistsResponse = await axios.get(`${connectionDetails.socket}/mgmt/filestore/${domain}/local/${fileName}`, {
			headers: {
				'Authorization': `Basic ${connectionDetails.authorization}`
			}
		});
		...
		if (fileExistsResponse.status === 200) {
			...
			const fileUpdateResponse = await axios.put(`${connectionDetails.socket}/mgmt/filestore/${domain}/local/${fileName}`, fileUpdateBody, {
				headers: {
					'Authorization': `Basic ${connectionDetails.authorization}`
				}
			}).catch(error => {
		...
		const response = await axios.post(`${connectionDetails.socket}/mgmt/filestore/${domain}/local`, fileBody, {
			headers: {
				'Authorization': `Basic ${connectionDetails.authorization}`
			}
		});
...
async function getDataPowerFileManagementStatus(domain, connectionDetails) {
	console.log({ domain, connectionDetails })
	if (!connectionDetails || !domain) {
		return undefined;
	}
	try {
		const fileManagmentResponse = await axios.get(`${connectionDetails.socket}/mgmt/filestore/${domain}/local`, {
			headers: {
				'Authorization': `Basic ${connectionDetails.authorization}`
			}
		});
